    ...
    def cmake_args(self):
        ...
        with working_dir('spack-build', create=True):
            ...
            if "+mpi" in spec:
                ...
                options.append("-DCMAKE_CXX_COMPILER:PATH=%s" % spec['mpi'].mpicxx)
                options.append("-DCMAKE_C_COMPILER:PATH=%s" % spec['mpi'].mpicc)
                options.append("-DCMAKE_Fortan_COMPILER:PATH=%s" % spec['mpi'].mpifc)
                options.append("-DMPIEXEC_EXECUTABLE:PATH=%s" % join_path(spec['mpi'].prefix.bin, 'mpiexec'))
            else:
